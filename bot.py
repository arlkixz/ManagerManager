...
async def handle_url(msg: Message):
    ...
    try:
        ...
        if ext in (".mp3", ".m4a", ".ogg", ".flac", ".wav", ".opus"):
            ...
            await msg.reply_audio(media_file, caption=f"🎵 {bot_username}")
        else:
            # Send video
            media_file = FSInputFile(file_path, filename=file_path.name)
            ...
            # Also send audio separately
            await status.edit_text("🎵 در حال استخراج صدا...")
            audio_path, audio_err = await download_media(url, audio_only=True)
            if audio_path:
                audio_file = FSInputFile(audio_path, filename=audio_path.name)
                await msg.reply_audio(audio_file, caption=f"🎵 {bot_username}")
                cleanup(audio_path)
