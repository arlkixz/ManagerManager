...
async def clean_week(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # دریافت شیء chat با await
        chat = await context.bot.get_chat(chat_id)
        
        async for message in chat.iterate_messages(limit=500):
            checked_count += 1
...
async def clean_new(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async for message in chat.iterate_messages(limit=count+10):
...
async def clean_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async for message in chat.iterate_messages(limit=100):
