...
async def download_media(url: str, audio_only: bool = False) -> tuple[Path | None, str | None]:
    ...
    try:
        ...
        downloaded = list(DOWNLOAD_DIR.glob(Path(base_path).name + "*"))
        if not downloaded:
        ...
        if size_mb > MAX_FILE_SIZE_MB:
            ...
            return None, (
                f"❌ حجم فایل {size_mb:.1f}MB از حد مجاز {MAX_FILE_SIZE_MB}MB بیشتره.\n"
                "برای فیلم‌های بزرگ، لینک مستقیم رو بفرست."
            )
