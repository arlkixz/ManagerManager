...
from telethon.tl.functions.messages import DeleteMessagesRequest, ReportSpamRequest
from telethon.tl.types import ChatBannedRights, ChatAdminRights, MessageEntityCustomEmoji, InputMessagesFilterEmpty
...
def get_target(event):
    if event.reply_to_msg_id:
        return event.reply_to_msg_id.sender_id if hasattr(event.reply_to_msg_id, "sender_id") else None
    if event.pattern_match and len(event.pattern_match.group(0).split()) > 1:
        try:
            return int(event.pattern_match.group(0).split()[1])
        except:
            return None
    return None
