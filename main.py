#!/usr/bin/env python3
"""
ربات دانلودر حرفه‌ای - دانلود صحیح MP3 و ویدیو با صدا
"""
    ...
    for i, q in enumerate(qualities):
        if q['type'] == 'video':
            ...
            keyboard.append([InlineKeyboardButton(
                ...
                callback_data=f"vid_{i}"
            )])
        else:
            keyboard.append([InlineKeyboardButton(
                f"🎵 {q['quality']}",
                callback_data=f"aud_{i}"
            )])
...
def download_video(url: str, format_id: str) -> Optional[str]:
    ...
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            # اگر MP4 نیست، تبدیل کن
            if not downloaded_file.endswith('.mp4'):
                final_file = downloaded_file.rsplit('.', 1)[0] + '.mp4'
                if convert_to_mp4_with_audio(downloaded_file, final_file):
...
📥 **قابلیت‌ها:**
• دانلود ویدیو با صدا (MP4)
• دانلود فقط صدا (MP3)
• پشتیبانی از 1500+ سایت

🎯 **نحوه استفاده:**
لینک رو بفرست، کیفیت رو انتخاب کن.
"""
...
async def handle_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not qualities:
        await msg.edit_text("❌ هیچ فرمتی یافت نشد!")
        return
...
async def quality_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تشخیص نوع درخواست (ویدیو یا صدا)
