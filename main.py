...
# Telethon برای کار با اکانت واقعی (UserBot)
user_client = TelegramClient('digital_persona_session', API_ID, API_HASH)

# Aiogram برای بات مدیریت (Admin Bot)
...
- هیچوقت جواب طولانی و کتابی نده""",
            "response_chance": 70,  # درصد احتمال پاسخ در گروه‌ها
            "enabled_chats": [],    # لیست چت‌های فعال (خالی یعنی همه)
            "blacklist_chats": [],  # لیست چت‌های ممنوع
            "cooldown_seconds": 60  # فاصله بین پاسخ‌ها (ثانیه)
        }
...
async def handle_new_message(event):
    try:
        # اگه پیام از خودمون باشه، نادیده بگیر
        ...
        if response_text:
            # شبیه‌سازی تایپ کردن (واقعی‌تر به نظر میاد)
            async with user_client.action(chat_id, 'typing'):
                await asyncio.sleep(random.uniform(1, 3))
            
            # ارسال پاسخ
            await user_client.send_message(chat_id, response_text)
            logger.info(f"پاسخ ارسال شد به چت {chat_id}: {response_text[:50]}...")
    
    except Exception as e:
        logger.error(f"خطا در هندلر پیام: {e}")

# ========== بات مدیریت (برای تنظیمات) ==========
...
💡 برای تغییر تنظیمات، از منوی اصلی استفاده کن."""
...
# ========== اجرای اصلی ==========
async def main():
    # استارت کردن هر دو کلاینت
    await user_client.start()
    logger.info("✅ User Client (اکانت شخصی) متصل شد")
    
    # استارت بات مدیریت
    from aiogram import executor
    executor.start_polling(dp, skip_updates=True)
    
    logger.info("✅ ربات شخصیت با موفقیت راه‌اندازی شد!")
